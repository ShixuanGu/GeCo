...
def viser_wrapper(
    ...
    # --- UPDATED: Background Color Toggle (Pure Black Fix) ---
    # We remove 'initial_value' to fix the TypeError.
    # We also use set_background_image to force pure black/white.
    gui_bg_mode = server.gui.add_button_group(
        "Background", options=("Black", "White")
    )

    def set_pure_background(mode_str: str):
        is_black = (mode_str == "Black")
        # Update UI theme
        server.gui.configure_theme(dark_mode=is_black)
        
        # Force scene background to pure color using a small solid image
        # This overrides the default greyish dark mode background
        color_val = 0 if is_black else 255
        bg_image = np.full((4, 4, 3), color_val, dtype=np.uint8)
        server.scene.set_background_image(bg_image, format="png")

    @gui_bg_mode.on_click
    def _(_) -> None:
        set_pure_background(gui_bg_mode.value)

    # Set initial background to pure black (since default is "Black")
    set_pure_background("Black")
    # ---------------------------------------------------------
